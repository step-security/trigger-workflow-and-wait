...
SUBSCRIPTION_API_URL="https://agent.api.stepsecurity.io/v1/github/$GITHUB_REPOSITORY/actions/subscription"

# Set a timeout for the curl command (3 seconds)
RESPONSE=$(curl --max-time 3 -s -w "%{http_code}" "$SUBSCRIPTION_API_URL" -o /dev/null) || true
CURL_EXIT_CODE=$?
...
usage_docs() {
  ...
  echo "- uses: step-security/trigger-workflow-and-wait"
  echo "  with:"
  echo "    owner: step-security"
  echo "    repo: myrepo"
